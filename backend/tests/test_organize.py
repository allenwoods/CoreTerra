...
def test_organize_enforces_accountability(client, temp_workspace):
    ...
    t = client.post(
        "/tasks/",
        json={"title": "Unowned Task", "user_id": TEST_USER_ID, "type": "Capture"},
    ).json()
    ...
    assert resp.status_code in [400, 422], (
        "System should reject activating a task without a designated Role Owner"
    )
    ...
    organize_payload = {
        ...
        "updated_at": t[
            "updated_at"
        ],  # Use original timestamp as previous call failed (no update happened)
    }
...
def test_organize_defines_nature_of_work(client, temp_workspace):
    ...
    """
    t = client.post(
        "/tasks/", json={"title": "Ref", "user_id": TEST_USER_ID, "type": "Capture"}
    ).json()
