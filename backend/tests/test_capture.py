...
def test_capture_generates_unique_identity(client, temp_workspace):
    ...
    payload = TaskCreateRequest(
        title="Same Idea", user_id=TEST_USER_ID, body="Content", type=TaskType.CAPTURE
    ...
    assert task1["id"] != task2["id"], (
        "Each captured task must have a unique global identifier (task_id)"
    )
...
def test_capture_establishes_timeline(client, temp_workspace):
    ...
    """
    payload = TaskCreateRequest(
        title="Timeline Test", user_id=TEST_USER_ID, type=TaskType.CAPTURE
    ).model_dump(mode="json")
    ...
    assert task["capture_timestamp"] is not None, (
        "Captured task must have a timestamp to serve as the baseline for PQI metrics"
    )
    assert task["status"] == "inbox", (
        "Newly captured tasks must land in Inbox to await processing"
    )
...
def test_capture_with_full_metadata(client, temp_workspace):
    ...
    assert task["tags"] == ["urgent", "bug", "frontend"], (
    ...
    assert task["status"] == "inbox", (
        "Even with role_owner set, new tasks start in inbox"
    )
