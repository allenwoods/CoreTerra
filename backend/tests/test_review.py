...
def test_review_establishes_priority_hierarchy(client, temp_workspace):
    ...
    if len(tasks) >= 2:
        assert tasks[0]["id"] == t2["id"], f"Highest priority task ({t2['id']}) should be first, got {tasks[0]['id']}"
        assert tasks[1]["id"] == t1["id"], f"Lowest priority task ({t1['id']}) should be second, got {tasks[1]['id']}"
...
def test_review_creates_commitment(client, temp_workspace):
    ...
    assert actual == expected, "Review must lock in a commitment date for CAR calculation"
